...
def load_tracks(output_dir="outputs"):
    ...
    return Tracks(
        image_ball, dict(player_boxes), sorted(ball_court, key=lambda point: point.frame),
        {key: sorted(value, key=lambda point: point.frame) for key, value in player_court.items()},
        valid_frames, net_points, last_frame,
    )